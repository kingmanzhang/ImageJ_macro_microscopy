...
//      setBatchMode(true);
  ...
  if (endsWith(list[i], "/")){
	...
	run("Bio-Formats Importer", "open=["+path+"] default open_all_series split_channels view=[Standard ImageJ] stack_order=Default");
	...
	if (nImages==3){
		...
		run("Merge Channels...", "red=["+list[i]+ " - C=1] green=["+list[i]+ " - C=0] blue=["+list[i]+" - C=2] gray=*None*");
